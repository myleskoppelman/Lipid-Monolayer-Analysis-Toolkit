...
def isolateDomain(data_path, tif_path):
    """
    ...
    save_path = easygui.filesavebox(
        ...
        default=os.path.join(path, f"{nm}_DOMAIN{settings[0]}.tif"),
        filetypes=["*.tif"]
    )
    if not save_path:
    ...
    data_save_path = easygui.filesavebox(
        ...
        default=os.path.join(path, f"{nm}_DOMAIN{settings[0]}.xlsx"),
        filetypes=["*.xlsx"]
    )
...
def main():
    ...
    isolateDomain(data_path, tif_path)
