...
''' [cagedDomains.py] Last Updated: 6/19/2025 by Myles Koppelman '''


def getFiles():
    _ = easygui.buttonbox(
        msg="Step 1: Chose color of Inner Domain for tracking",
        title="Settings",
        choices=["Continue"]
        )
    inner_tif_path, inner_tagged_tif_path, inner_tracked_xlsx_path = trackAndTagComplete.main()
    
    
    _ = easygui.buttonbox(
        msg= "Step 2: Chose color of Outer Domain for tracking",
        title="Settings",
        choices=["Continue"]
        )
    outer_tif_path, outer_tagged_tif_path, outer_tracked_xlsx_path = trackAndTagComplete.main()
    
    return inner_tif_path, inner_tagged_tif_path, inner_tracked_xlsx_path, outer_tif_path, outer_tagged_tif_path, outer_tracked_xlsx_path
...
def isPointInEllipse(x, y, cx, cy, a, b, angle_deg):
    """
    Returns True if (x, y) is inside the rotated ellipse centered at (cx, cy),
    with semi-axes a and b, and rotated by angle_deg degrees counterclockwise.
    """
...
def calculateEta(xlsx_path):
    ...
    for _, row in df_all.iterrows():
        ...
        rho = float(row["Centroid_Distance"]) * 1e-6 * 0.2222
        
        inner_r = np.sqrt(inner_area / np.pi)
        if inner_r < 4:
            continue
        inner_r = inner_r * 1e-6 * 0.2222
        outer_r = np.sqrt(outer_area / np.pi) * 1e-6 * 0.2222
    ...
    return eta_scaled, eta_bar, hist, bin_edges, fit
